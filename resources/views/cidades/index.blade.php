...
<style>
    ...
    .page-hero p { color: #a0a0c0; font-size: 16px; margin: 0; }
    ...
    .city-card:hover .city-arrow { color: #4a4aff; transform: translateX(4px); }
    ...
    @media(max-width: 640px) {
        ...
        .search-box, .tab-bar, .content-area { padding-left: 16px; padding-right: 16px; }
...
<div class="search-section">
    <div class="container">
        ...
        <div class="tab-bar">
            <button class="tab-btn active" data-tab="todas">Todas as cidades</button>
            @if(auth()->user()->cidades && auth()->user()->cidades->count() > 0)
            <button class="tab-btn" data-tab="minhas">Minhas cidades</button>
            @endif
        </div>
    </div>
</div>
...
<div style="background:#fff;">
    <div class="container">
        <div class="content-area">
            ...
            <div id="tab-todas">
                <p class="section-label">{{ $cidades->count() }} cidade{{ $cidades->count() != 1 ? 's' : '' }} disponível</p>
                ...
                <div class="city-grid" id="grid-cidades">
                    ...
                    <a href="{{ route('espacos.index', $cidade->id) }}" class="city-card" data-nome="{{ strtolower($cidade->nome) }}">
                        <div class="city-icon">🏙️</div>
                        <div class="city-name">{{ $cidade->nome }}</div>
                        <div class="city-uf">
                            <span class="badge-uf">{{ $cidade->uf }}</span>
                            {{ $cidade->espacos_count ?? $cidade->espacos->count() }} espaço(s)
                        </div>
                        <span class="city-count">Ver espaços</span>
                        <span class="city-arrow"><i class="bi bi-arrow-right"></i></span>
                    </a>
                    @endforeach
                </div>
                @endif
    ...
    input.addEventListener('input', function () {
        ...
        if (found === 0 && q) {
            if (!empty) {
                ...
                empty.innerHTML = '<div class="icon">🔍</div><p>Nenhuma cidade encontrada para "<strong>' + q + '</strong>"</p>';
